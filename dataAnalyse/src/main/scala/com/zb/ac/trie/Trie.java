...
import com.zb.ac.interval.IntervalTree;
...
public class Trie implements Serializable{
	 ...
	 * 
	 */
	private static final long serialVersionUID = 5996323740890077903L;
    ...
    public void addKeyword(String keyword, String labkey) {
        ...
        currentState.addEmit(keyword, labkey);
    }
    ...
    private void storeEmits(int position, State currentState, List<Emit> collectedEmits) {
        ...
        if (emits != null && !emits.isEmpty()) {
            for (KeyWord emit : emits) {
                collectedEmits.add(new Emit(position-emit.getKeyword().length()+1, position, emit.getKeyword(), emit.getLabkey()));
            }
        }
    }
