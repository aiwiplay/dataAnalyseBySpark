...
public class State implements Serializable{
	 ...
	 * 
	 */
	private static final long serialVersionUID = -4974502308712359369L;
    ...
    public void addEmit(String keyword, String labkey) {
        if (this.emits == null) {
            this.emits = new TreeSet<KeyWord>();
        }
        this.emits.add(new KeyWord(keyword, labkey));
    }

    public void addEmit(Collection<KeyWord> emits) {
        for (KeyWord emit : emits) {
            addEmit(emit.getKeyword(), emit.getLabkey());
        }
    }
